      ...
      <div className="w-full max-w-md bg-card shadow-xl shadow-black/50/50 border border-white/10 rounded-3xl overflow-hidden shadow-2xl">
        <div className="p-6 border-b border-white/10">
          <div className="flex items-center justify-between">
            ...
            <button onClick={onClose} className="text-foreground hover:text-[#025964] transition-colors"><X className="h-5 w-5" /></button>
          </div>
          <p className="text-xs text-foreground mt-1">This action cannot be undone.</p>
        </div>
        <div className="p-6 space-y-4">
          {error && <div className="p-3 bg-[#fde7e9] border border-red-100 rounded-xl text-[#ee1123] text-xs flex items-center gap-2"><AlertCircle className="h-4 w-4" />{error}</div>}
          <div className="p-4 bg-card shadow-xl shadow-black/50/50/10 border border-[#028597]/20/20 rounded-xl">
          ...
          <div className="space-y-2">
            ...
            <textarea rows={3} value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Help us improve by sharing why you're cancelling..." className="w-full bg-card border border-white/10 focus:border-[#028597]/20 rounded-xl px-4 py-3 text-sm text-foreground outline-none placeholder:text-foreground resize-none" />
          </div>
        </div>
        <div className="p-6 border-t border-white/10 flex items-center justify-end space-x-3">
          ...
          <button onClick={handleCancel} disabled={loading} className="px-5 py-2.5 bg-emergency-red hover:bg-[#ee1123] text-foreground text-xs font-bold rounded-xl transition-colors flex items-center gap-2">
